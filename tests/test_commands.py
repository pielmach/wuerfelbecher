...
class TestCommands(unittest.TestCase):
    ...
    def test_roll(self):
        ...
        self.assertTrue(commands.roll('2d2-1') in ['[ **1  1** ]-1=**1**', '[ **1  2** ]-1=**2**',
                                                   '[ **2  1** ]-1=**2**', '[ **2  2** ]-1=**3**'])
        self.assertTrue(commands.roll('2d2+') in ['[ **1  1** ]=**2**', '[ **1  2** ]=**3**',
                                                  '[ **2  1** ]=**3**', '[ **2  2** ]=**4**'])
        self.assertTrue(commands.roll('2d2+0') in ['[ **1  1** ]=**2**', '[ **1  2** ]=**3**',
                                                   '[ **2  1** ]=**3**', '[ **2  2** ]=**4**'])
        self.assertTrue(commands.roll('d2 d2') in ['[ **1** ]  [ **1** ]', '[ **1** ]  [ **2** ]',
                                                   '[ **2** ]  [ **1** ]', '[ **2** ]  [ **2** ]'])
        self.assertTrue(commands.roll(' '.join(['d2', 'd2'])) in ['[ **1** ]  [ **1** ]', '[ **1** ]  [ **2** ]',
                                                                  '[ **2** ]  [ **1** ]', '[ **2** ]  [ **2** ]'])
